      ...
      if (!isTestMode) {
        try {
          await db.insert(generatedAsset).overridingSystemValue().values({
            ...
            styleId: typeof style === 'string' ? style : null,
            videoStyle: typeof style === 'string' ? style : null,
            r2Key: `polling-failed-${taskId ?? 'unknown'}`,
            publicUrl: '',
            status: 'failed',
      ...
      if (!isTestMode) {
        try {
          await db.insert(generatedAsset).overridingSystemValue().values({
            ...
            styleId: typeof style === 'string' ? style : null,
            videoStyle: typeof style === 'string' ? style : null,
            r2Key: `no-video-url-${taskId ?? 'unknown'}`,
            publicUrl: '',
            status: 'failed',
        ...
        if (!isTestMode) {
          try {
            await db.insert(generatedAsset).overridingSystemValue().values({
              ...
              styleId: typeof style === 'string' ? style : null,
              videoStyle: typeof style === 'string' ? style : null,
              r2Key: `download-failed-${taskId ?? 'unknown'}`,
              publicUrl: videoResult.videoUrl,
              status: 'failed',
              errorMessage: errorMsg,
              creditsSpent: 0,
              generationParams: {
      ...
      if (!isTestMode) {
        try {
          await db.insert(generatedAsset).overridingSystemValue().values({
            ...
            styleId: typeof style === 'string' ? style : null,
            videoStyle: typeof style === 'string' ? style : null,
            r2Key: `download-exception-${taskId ?? 'unknown'}`,
            publicUrl: videoResult.videoUrl,
            status: 'failed',
            errorMessage: errorMessage,
            creditsSpent: 0,
      ...
      if (!isTestMode) {
        try {
          await db.insert(generatedAsset).overridingSystemValue().values({
            ...
            styleId: typeof style === 'string' ? style : null,
            videoStyle: typeof style === 'string' ? style : null,
            r2Key: `upload-failed-${taskId ?? 'unknown'}`,
            publicUrl: videoResult.videoUrl ?? '',
            status: 'failed',
            errorMessage: errorMessage,
            creditsSpent: 0,
