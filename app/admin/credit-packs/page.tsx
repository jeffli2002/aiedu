  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(`Failed to load credit pack stats (HTTP ${response.status}).`);
      }
    } catch (error) {
      console.error('Failed to fetch credit pack stats:', error);
      setError('Failed to fetch credit pack stats. Please try again.');
    } finally {
      setIsLoading(false);
    }
  }, [range]);
    ...
    return (
      <div className="p-6">
        <div className="max-w-xl mx-auto">
          <Card>
            <CardHeader>
              <CardTitle>Credit Pack Data Unavailable</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <p className="text-sm text-slate-600">
                {error ?? 'No credit pack data available yet.'}
              </p>
