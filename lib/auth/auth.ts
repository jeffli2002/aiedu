...
const createTrustedOrigins = (): string[] => {
  ...
  try {
    const appUrl = env.NEXT_PUBLIC_APP_URL || 'http://localhost:3003';
...
export const auth = betterAuth({
    ...
    sendVerificationEmail: async ({ user, url }) => {
      try {
        ...
        try {
          ...
          console.warn('[auth] Failed to check verification throttle:', dbError);
        ...
        if (sent) {
          ...
          try {
            ...
            // Don't throw - throttle record failure shouldn't block signup
          }
        } else {
        ...
        console.error(`[auth] Error in sendVerificationEmail for ${user.email}:`, error);
      }
    },
  ...
  rateLimit: {
    enabled: true,
    storage: 'database', // Requires 'rateLimit' table in database
    window: 60 * 60,
    max: 100,
    customRules: {
      '/sign-up/email': {
