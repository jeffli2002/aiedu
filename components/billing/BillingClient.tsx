...
const BillingClient = ({ plans }: BillingClientProps) => {
  ...
  useEffect(() => {
    ...
    if (successFlag === 'true' && checkoutId && !subscriptionId && !isSyncingCheckout) {
      setIsSyncingCheckout(true);
      fetch('/api/creem/sync-credit-pack', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        ...
        .then(async () => {
          toast.success('Credit pack synced');
          await fetchSubscription();
        })
        .catch((error) => {
          toast.error(
            error instanceof Error ? error.message : 'Failed to sync credit pack. Please refresh.'
          );
        })
        .finally(() => {
          setIsSyncingCheckout(false);
